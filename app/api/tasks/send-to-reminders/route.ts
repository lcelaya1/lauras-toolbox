...
// Creates reminders in the default list (no list name needed).
...
export async function POST(req: NextRequest) {
  ...
  const script = `tell application "Reminders"${remindersBlock}\nend tell`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: "osascript failed — only works when running locally on macOS", detail: message },
      { status: 500 },
    );
  }
}
